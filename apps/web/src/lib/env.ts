...
const clientEnvSchema = z.object({
  ...
  NEXT_PUBLIC_CONTRACT_LICENSE_ADDRESS: z
    .string()
    .regex(/^0x[a-fA-F0-9]{40}$/, "Invalid license contract address"),
  NEXT_PUBLIC_WALLETCONNECT_ID: z.string().min(1),
  NEXT_PUBLIC_STORAGE_MODE: z.enum(["local", "production"]).default("local"),
  NEXT_PUBLIC_CONTRACT_COMMITTEE_ADDRESS: z
    .string()
...
export const clientEnv = clientEnvSchema.parse({
  ...
  NEXT_PUBLIC_CONTRACT_LICENSE_ADDRESS: process.env.NEXT_PUBLIC_CONTRACT_LICENSE_ADDRESS,
  NEXT_PUBLIC_WALLETCONNECT_ID: process.env.NEXT_PUBLIC_WALLETCONNECT_ID,
  NEXT_PUBLIC_STORAGE_MODE: process.env.NEXT_PUBLIC_STORAGE_MODE,
  NEXT_PUBLIC_CONTRACT_COMMITTEE_ADDRESS: process.env.NEXT_PUBLIC_CONTRACT_COMMITTEE_ADDRESS,
